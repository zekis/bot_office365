...
import langchain
langchain.debug = ai_config.VERBOSE
...
class openaai_master:
    ...
    async def async_init(self):
        #Check Config
        credentials = CredentialManager(ai_config.DATA_DIR, ai_config.USER_ID)
        credentials.load_credentials()
        await self.config_graph(credentials)
        await self.config_openai(credentials, ai_config.RESET_CONFIG)
        ai_config.RESET_CONFIG = False
        publish("config: " + credentials.to_json(ai_config.VERBOSE))
        publish("model: " + ai_config.MAIN_AI + ", tool: " + ai_config.TOOL_AI)
    ...
    async def config_graph(self, credentials):
        ...
        credentials.add_credential('graph_api', {"application_id": ai_config.APP_ID, "application_password": ai_config.APP_PASSWORD, "tenantID": ai_config.TENANT_ID, "main_resource": ai_config.OFFICE_USER} )
        credentials.save_credentials()

        

    async def config_openai(self, credentials, reset=False):
        #GPT API Key
        if reset:
        ...
        if cred:
            ...
            ai_config.OPENAI_API_KEY = cred_openAI_api_key['parameters']['key']
            os.environ["OPENAI_API_KEY"] = ai_config.OPENAI_API_KEY
        else:
            #No openai api key, Better get the key before doing anything
            publish_input_card("Please enter your OpenAI API Key", [{"label": "OpenAI API Key (Type 'config' if you need to change this later)", "id": "config_value"}] )
            response = await get_input(timeout_minutes=0)

            print("got response " + response)
            if response:
                
                credentials.add_credential('openai_api', {"key": response})
                ai_config.OPENAI_API_KEY = response
                os.environ["OPENAI_API_KEY"] = ai_config.OPENAI_API_KEY
                credentials.save_credentials()
                publish("Config updated. restart bot using bot_restart")
                #"restart", user_id, tenant_id, user_name, email_address
                publish_system_message("restart", {'user_id': ai_config.USER_ID, 'tenant_id': ai_config.TENANT_ID, 'user_name': ai_config.FRIENDLY_NAME, 'email_address': ai_config.OFFICE_USER})
            else:
        ...
        try:
            ...
            if msg:
                ...
                if question == 'memory':
                    ...
                    #publish("chat_history: " + self.chat_history + "\n")
                    publish("memory_buffer: " + self.memory.buffer_as_str + "\n")
                elif question == 'die':
                    publish("Finally..")
                    exit()
    ...
    def process_task_schedule(self):
        while True:
            task = scheduler_get_task_due_today(ai_config.Todo_BotsTaskFolder)
            if not task:
                break
            else:
        ...
        while True:
            task = scheduler_get_bots_unscheduled_task(ai_config.Todo_BotsTaskFolder)
            if not task:
                break
            else:
                publish(f"Looks like one of my tasks is due - {task.subject}")
