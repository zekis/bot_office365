...
def get_email_summary(email, body_soup):
    try:
        chat = ChatOpenAI(temperature=0, model_name=ai_config.TOOL_AI)
...
def reply_to_email_summary(summary, comments=None, previous_draft=None):
    chat = ChatOpenAI(temperature=0, model_name=ai_config.TOOL_AI)
    query = f"""Given this email summary: {summary}, please create a reasonable email response from 'Chad the AI Assistant' on behalf of {ai_config.OFFICE_USER}.
...
def forward_email_summary(summary, comments=None, previous_draft=None):
    chat = ChatOpenAI(temperature=0, model_name=ai_config.TOOL_AI)
    query = f"""Given this email summary: {summary}, please create a reasonable email from 'Chad the AI Assistant' on behalf of {ai_config.OFFICE_USER}.
    Response is to be HTML formatted and must include an informal 'To' salutation and opening line at the start and add a signature from 'Chad the AI Assistant'
    """
    if comments:
        query += f"Consider the following comments: {comments}"
...
def modify_draft(body, comments, previous_draft=None):
    chat = ChatOpenAI(temperature=0, model_name=ai_config.TOOL_AI)
    query = f"""Given this request: {body}, please create a reasonable email from 'Chad the AI Assistant' on behalf of {ai_config.OFFICE_USER}
    ...
    if emails:
        for email in emails:
            if not email.is_event_message:
                ...
                email.mark_as_read()
                send_to_bot(ai_config.USER_ID,"Only If the following email requires " + ai_config.FRIENDLY_NAME + " to perform an action such as reply, send a file, fix a problem, complete work etc, then use CREATE_TASK to create a new task in the Tasks folder. Email: " + ai_summary)
            else:
        ...
        try:
            ...
            if emails:
                ...
                if publish.lower() == "true":
                    ...
                    #return ai_config.PROMPT_PUBLISH_TRUE
                else:
    ...
    def _run(self, EmailID: str = None, ConversationID: str = None, follow_up: bool = True, publish: str = "True", run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        try:
            ...
            if email:
                ...
                task_prompt = f"""Only if the following message is directed at {ai_config.FRIENDLY_NAME}, 
                draft email reply with basic troubleshooting steps using the DRAFT_REPLY_TO_EMAIL tool with 'ConversationID' {email.conversation_id} and 'user_improvements' with HTML formated troubleshooting tips. No need to use the GET_EMAIL_CHAIN. Examples of troubleshooting steps might include checking internet connection, restarting the application, or verifying login credentials.
                If you, the AI, do not have any suggestions then use the CREATE_TASK tool to generate a new task in the Tasks folder. 
                ********************
                Message: {ai_summary}
                ********************
                """
                ...
                if publish.lower() == "true":
                    ...
                    if follow_up:
                        send_to_bot(ai_config.USER_ID,task_prompt)
                    self.return_direct = True
                    return None
                else:
                    if follow_up:
